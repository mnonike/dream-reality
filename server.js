...
function generateReferralCode(firstname) {
  ...
  const randomPart = Array(6).fill(0).map(() => chars.charAt(Math.floor(Math.random() * chars.length))).join('');
  return (firstname.substring(0, 3) + randomPart).toUpperCase();
}

// Helper function to validate referral code
function validateReferralCode(referralCode, usersData) {
  return usersData.users.find(user => user.referralCode === referralCode);
}
...
app.post('/api/register', (req, res) => {
  try {
    const { username, password, firstname, phone, referral } = req.body;
    ...
    if (referral && referral.trim() !== '') {
      const referrer = validateReferralCode(referral, usersData);
      if (referrer) {
...
app.get('/api/disclosures', (req, res) => {
  try {
    // Initialize empty disclosures if file doesn't exist
    if (!fs.existsSync(path.join(DATA_DIR, 'disclosures.json'))) {
      fs.writeFileSync(path.join(DATA_DIR, 'disclosures.json'), JSON.stringify({ disclosures: [] }, null, 2));
    }
    
    const disclosuresData = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'disclosures.json'), 'utf8'));
    res.json(disclosuresData);
  } catch (err) {
    res.status(500).json({ error: 'Failed to read disclosures' });
  }
});
...
app.post('/api/disclosures', (req, res) => {
  try {
    ...
    disclosuresData.disclosures.unshift(newDisclosure);
    fs.writeFileSync(path.join(DATA_DIR, 'disclosures.json'), JSON.stringify(disclosuresData, null, 2));
    
    // Emit new disclosure to all clients
    io.emit('new-disclosure', newDisclosure);
    
    res.json({ success: true, disclosure: newDisclosure });
  } catch (err) {
    res.status(500).json({ error: 'Failed to submit disclosure' });
  }
});

app.post('/api/disclosures/:id/reply', (req, res) => {
  try {
    const { id } = req.params;
    ...
    const disclosuresData = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'disclosures.json'), 'utf8'));
    const disclosureIndex = disclosuresData.disclosures.findIndex(d => d.id === id);
    
    if (disclosureIndex === -1) {
      return res.status(404).json({ error: 'Disclosure not found' });
    }
    
    disclosuresData.disclosures[disclosureIndex].adminReply = reply;
    disclosuresData.disclosures[disclosureIndex].replyDate = new Date().toISOString();
    
    fs.writeFileSync(path.join(DATA_DIR, 'disclosures.json'), JSON.stringify(disclosuresData, null, 2));
    
    // Emit reply to all clients
    io.emit('disclosure-replied', disclosuresData.disclosures[disclosureIndex]);
    
    res.json({ success: true, disclosure: disclosuresData.disclosures[disclosureIndex] });
  } catch (err) {
...
app.put('/api/user/:username', (req, res) => {
  try {
    ...
    res.json({ success: true, user: usersData.users[userIndex] });
  } catch (err) {
...
app.get('/api/user/:username/referrals', (req, res) => {
  try {
    ...
    const referrals = usersData.users.filter(u => u.referredBy === username);
    
    // Find who referred this user
    const currentUser = usersData.users.find(u => u.username === username);
    const referredBy = currentUser && currentUser.referredBy ? 
      usersData.users.find(u => u.username === currentUser.referredBy) : null;
    
    res.json({
      totalReferrals: referrals.length,
      activeReferrals: referrals.filter(u => u.isActive).length,
